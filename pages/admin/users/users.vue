<!-- eslint-disable camelcase -->
<template>
  <v-container fluid color="error">
    <v-snackbar v-model="snackbar">
      {{ textSnackbar }}

      <template v-slot:action="{ attrs }">
        <v-btn color="pink" text v-bind="attrs" @click="snackbar = false">
          Close
        </v-btn>
      </template>
    </v-snackbar>
    <v-dialog v-model="dialogDelete" width="unset">
      <v-sheet outlined color="error" rounded>
        <v-card>
          <v-card-title class="h5 red--text"
            >Bạn có chắc vô hiệu hóa không?</v-card-title
          >
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="error" text @click="closeDelete">Không</v-btn>
            <v-btn color="error" text @click="deleteUserConfirm">Có</v-btn>
            <v-spacer></v-spacer>
          </v-card-actions>
        </v-card>
      </v-sheet>
    </v-dialog>
    <v-card>
      <v-card-title style="left: 50%" class="red--text font-weight-bold"
        >QUẢN LÝ TÀI KHOẢN NGƯỜI XEM WMOVIES
      </v-card-title>
      <v-card-text>
        <v-data-table
          :headers="headers"
          :items="users"
          :footer-props="{
            itemsPerPageText: 'số dòng mỗi trang',
            pageText: '{0}-{1} trên {2}',
          }"
          item-key="id"
          class="elevation-1"
          loading-text="Đang tải...vui lòng đợi"
        >
          <template #[`item.is_delete`]="{ item }">
            <v-simple-checkbox
              v-model="item.is_delete"
              disabled
            ></v-simple-checkbox>
          </template>

          <template #top>
            <v-snackbar v-model="snackbar">
              {{ textSnackbar }}

              <template v-slot:action="{ attrs }">
                <v-btn
                  color="pink"
                  text
                  v-bind="attrs"
                  @click="snackbar = false"
                >
                  Close
                </v-btn>
              </template>
            </v-snackbar>
            <v-toolbar flat>
              <v-text-field
                v-model="search"
                append-icon="mdi-magnify"
                label="Tìm tài khoản người dùng"
                single-line
                hide-details
                @keyup.enter="searchAll"
              ></v-text-field>
              <v-spacer></v-spacer>
              <v-btn outlined color="error" class="mb-2" @click="addUser()">
                Tạo tài khoản
              </v-btn>
            </v-toolbar>
          </template>

          <template #[`item.actions`]="{ item }">
            <v-icon small class="mr-2" @click="editUser(item)">
              mdi-pencil
            </v-icon>
            <v-icon small @click="deleteUser(item)"> mdi-delete </v-icon>
          </template>
          <template #no-data>
            <v-btn color="error" @click="loadDataUsers"> Làm mới </v-btn>
          </template>
        </v-data-table>
      </v-card-text>
    </v-card>
  </v-container>
</template>
<script>
// import axios from 'axios'
import gql from 'graphql-tag'
import moment from 'moment'
export default {
  layout: 'admin',
  name: 'Homepage',
  data() {
    return {
      snackbar: false,
      textSnackbar: '',
      search: '',
      dialogDelete: false,
      headers: [
        {
          text: 'STT',
          value: 'stt',
          align: 'start',

          class: 'error font-weight-medium text-h6 white--text',
        },

        {
          text: 'Tên người dùng',
          value: 'username',

          class: 'error font-weight-medium text-h6 white--text',
        },
        {
          text: 'Email',
          value: 'email',

          class: 'error font-weight-medium text-h6 white--text',
        },

        {
          text: 'Ngày tạo',
          value: 'created_at_string',

          class: 'error font-weight-medium text-h6 white--text',
        },
        {
          text: 'Vô hiệu hóa',
          value: 'is_delete',

          class: 'error font-weight-medium text-h6 white--text',
        },
        {
          text: '',
          value: 'actions',
          sortable: false,
          class: 'error font-weight-medium text-h6 white--text',
        },
      ],
      users: [],
      user: {
        id: null,
        username: null,
        email: null,
        created_at: null,
      },
      editedIndex: -1,
    }
  },
  mounted() {},
  watch: {},
  methods: {
    closeDelete() {
      this.dialogDelete = false
      this.editedIndex = -1
    },
    getTotalData(data) {},
    editUser(item) {
      this.$router.push({
        path: `/admin/users/users_edit`,
        query: { id: item.id },
      })
    },
    deleteUser(item) {
      this.editedIndex = this.users.indexOf(item)
      if (this.editedIndex !== -1) {
        this.dialogDelete = true
      } else {
        this.snackbar = true
        this.textSnackbar = 'Không tìm thấy tài khoản'
      }
    },
    loadDataUsers() {},
    deleteUserConfirm() {
      const getItem = this.users[this.editedIndex]
      const updateemployeeGraphl = gql` mutation MyMutation {
        update_users(
          where: { id: { _eq: "${getItem.id}" } }
          _set: {
            is_delete: true
          }
        ) {
          affected_rows
        }

      
      }`
      this.$apollo.mutate({
        mutation: updateemployeeGraphl,
        variables: {},
        // eslint-disable-next-line camelcase
        update: (store, { data: { update_users } }) => {
          // eslint-disable-next-line camelcase
          if (update_users.affected_rows) {
            this.$apollo.queries.getData.refetch()
            this.textSnackbar = 'Vô hiệu hóa tài khoản thành công'
            this.snackbar = true
            // this.users.splice(this.editedIndex, 1)
            this.closeDelete()
          } else {
            this.textSnackbar = 'Vô hiệu hóa tài khoản thất bại'
            this.snackbar = true
            this.closeDelete()
          }
        },
      })
    },
    addUser() {
      this.$router.push({
        path: `/admin/users/users_edit`,
        query: { id: 0 },
      })
    },
  },
  apollo: {
    getData: {
      query() {
        const query = gql(`query MyQuery {
             users {
                id
                username
                email
                password
                created_at
                is_delete
             }
          }`)
        return query
      },
      update: (data) => {},
      result({ data }) {
        this.users = []
        if (data && data.users && data.users.length > 0) {
          for (let index = 0; index < data.users.length; index++) {
            const element = data.users[index]
            element.stt = index + 1
            element.created_at_string = moment(element.created_at)
              .add('hours', 7)
              .format('DD-MM-YYYY HH:mm:ss')

            this.users.push(element)
          }
        }
      },
    },
  },
}
</script>

<style></style>
