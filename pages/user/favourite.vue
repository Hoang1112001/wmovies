              ...
              <v-col cols="12" sm="8" lg="8" xl="8" md="8">
                <v-app-bar color="error" class="">
                  <v-toolbar-title
                    ...
                    ><v-icon color="white">mdi-heart</v-icon>DANH SÁCH PHIM YÊU
                    THÍCH</v-toolbar-title
                  >
                </v-app-bar>
                ...
                <v-container>
                  <v-row dense>
                    ...
                    <v-col
                      v-else
                      v-for="(file, i) in favouriteMovies"
                      :key="i"
                      cols="12"
                      sm="3"
                      lg="3"
      ...
      if (this.isLogin) {
        try {
          const resData = await axios.get(
            `${process.env.URL_SERVER}/api/get-movie-favourite/${this.isLogin}/${this.limit}/${this.offset}`
          )
          if (resData && resData.data && resData.data.status) {
            if (resData.data && resData.data.dataFavourite.length > 0) {
              this.favouriteMovies = []
              const arrayCheck = []
              for (
                let index = 0;
                index < resData.data.dataFavourite.length;
                index++
              ) {
                const element = resData.data.dataFavourite[index].movie
                if (
                  arrayCheck.length === 0 ||
                  ...
                  this.favouriteMovies.push(movieItem)
                } else {
                  continue
                }
              }
